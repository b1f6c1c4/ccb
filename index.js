...
emlformat.read(eml, (error, data) => {
  ...
  const re = /^ (?<td>20[0-9][0-9]-[01][0-9]-[012][0-9])\s\s(?<pd>20[0-9][0-9]-[01][0-9]-[012][0-9])\s\s(?:[0-9]{4})\s\s(?<rmk>\S+(?:\s\S+)*)\s\s(?<tc>[A-Z]{3})\s\s(?<tf>-?[0-9]{1,3}(?:,[0-9]{3})*\.[0-9]{2})\s\s(?<pc>[A-Z]{3})\s\s(?<pf>-?[0-9]{1,3}(?:,[0-9]{3})*\.[0-9]{2})$/gm;
  let m;
